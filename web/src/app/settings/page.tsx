...
export default function SettingsPage() {
  ...
  return <div className="grid gap-6 xl:grid-cols-2">
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><KeyRound className="size-5" />{zh ? "我的 DeepSeek API Key" : "My DeepSeek API key"}</CardTitle><CardDescription>{zh ? "每个商家可以使用自己的模型账户。密钥加密存储，只在服务端调用，不会回显。" : "Each merchant can use their own model account. Keys are encrypted at rest and never displayed again."}</CardDescription></CardHeader><CardContent className="space-y-4"><p className="flex items-center gap-2 text-sm"><ShieldCheck className="size-4 text-emerald-600" />{configured ? (zh ? "已配置个人密钥" : "Personal key configured") : (zh ? "尚未配置个人密钥" : "No personal key configured")}</p><Input type="password" autoComplete="off" placeholder="sk-…" value={key} onChange={(event) => setKey(event.target.value)} /><div className="flex flex-wrap gap-2"><Button disabled={busy || !key.trim()} onClick={() => void save()}>{zh ? "保存密钥" : "Save key"}</Button>{configured && <Button variant="outline" disabled={busy} onClick={() => void save(true)}>{zh ? "删除个人密钥" : "Remove key"}</Button>}</div>{message && <p role="status" className="text-sm text-slate-600">{message}</p>}<p className="text-xs leading-5 text-slate-500">{zh ? "DeepSeek 用于联网检索、商家历史与市场信息的证据分析；利润金额仍由可复核的计算引擎生成。平台 API 凭据请在“平台连接”中管理。" : "DeepSeek examines public sources and shop history. Auditable calculations still produce profit numbers. Manage platform credentials under Connections."}</p></CardContent></Card>
