...
export default function QuickPage() {
  ...
  return <div className="space-y-6">
    <Card><CardHeader><CardTitle>先算一个商品</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-slate-600"><p>无需平台接口授权。请从店铺费率页、活动报名页、采购台账、物流账单和近期退款记录填写实际数据。所有输入只用于本次预览，不自动归档。</p><p>空白必填项必须填写；其余 0 表示你确认该项不适用。金额统一使用同一币种，费率按百分数填写。</p></CardContent></Card>
    <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm">平台<select className="mt-2 h-10 w-full rounded-md border bg-white px-3" value={platform} onChange={e=>setPlatform(e.target.value)}><option value="taobao">淘宝</option><option value="pinduoduo">拼多多</option><option value="douyin">抖音电商</option><option value="xianyu">闲鱼</option><option value="tiktok_shop">TikTok Shop</option><option value="amazon">Amazon</option><option value="temu">Temu</option><option value="shein">SHEIN</option><option value="other">其他</option></select></label><label className="text-sm">币种<input className="mt-2 h-10 w-full rounded-md border bg-white px-3" value={currency} maxLength={3} onChange={e=>setCurrency(e.target.value.toUpperCase())}/></label></div>
