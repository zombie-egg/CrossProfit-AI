...
export function AuthPage() {
  ...
  return <div className="relative h-svh min-h-[360px] overflow-hidden bg-white text-[#292823]">
    ...
    <div className="relative z-10 mx-auto grid h-full max-w-[2250px] grid-cols-1 items-center gap-6 px-5 pt-16 pb-4 sm:px-8 md:grid-cols-[minmax(0,1fr)_minmax(340px,44%)] md:gap-8 md:px-[4.5vw] md:py-12 xl:grid-cols-[minmax(0,1fr)_minmax(440px,32%)]">
      ...
      <section className="mx-auto flex h-auto max-h-[calc(100svh-88px)] w-full max-w-[540px] min-w-0 flex-col overflow-y-auto rounded-[15px] border border-[#dedbd4] bg-white/95 px-[clamp(18px,2.3vw,38px)] py-[clamp(20px,3svh,34px)] shadow-[0_2px_5px_rgba(50,45,35,0.11)] md:max-h-[calc(100svh-96px)]">
        ...
        <form onSubmit={(event) => void submit(event)} className="mt-[clamp(16px,3svh,32px)] flex flex-col gap-[clamp(8px,1.7svh,18px)]">
          ...
          <button type="submit" disabled={busy || ((mode === "login" || mode === "code") && !captcha)} className="mt-3 h-[clamp(42px,6svh,56px)] w-full shrink-0 whitespace-nowrap rounded-[10px] bg-[#292823] text-[clamp(15px,1.2vw,21px)] font-semibold text-white transition hover:bg-stone-700 disabled:opacity-60">{busy ? (zh ? "请稍候…" : "Please wait…") : title}</button>
        </form>
