...
from ..profit_engine import ProfitEngine, rate
...
class ReconciliationEngine:
    def run(self, snapshot_data: dict, lines: list[SettlementLine], fee_mapping: dict[str, str], unrecognized_columns: list[str] | None = None) -> dict:
        ...
        for (order_id, line_sku), group in grouped.items():
            ...
            same_inputs.platform_config.original_price = gross
            same_inputs.activity.discount_type = "none"
            same_inputs.activity.discount_value = Decimal("0")
            same_inputs.activity.platform_subsidy = subsidy
            ...
            same_inputs.activity.estimated_sales = 1
            expected = {item.item: item.amount for item in ProfitEngine().calculate(same_inputs).breakdown}
            ...
            for name, actual in actual_fees.items():
                ...
                differences.append({"order_id": order_id, "sku": sku, "fee_item": name,
                    ...
                    "reason": _reason(name, diff, currency_mismatch, refund > 0).value,
        ...
        actual_return = rate(Decimal(returned_orders) / Decimal(matched_orders)) if matched_orders else None
        forecast_diff = {"estimated_sales": estimated, "actual_sales": matched_orders, "sales_delta": matched_orders - estimated,
            ...
            "note": "预测偏差独立于公式正确性；退款窗口未结束时退货率仍可能变化。"}
        ...
        return {"formula_verdict": verdict, "max_fee_diff": str(max_diff), "forecast_diff": forecast_diff,
            "diff_data": {"fee_diffs": differences, "unmapped_fees": unmapped, "unmatched_orders": unmatched_orders,
                ...
                "assumptions": ["结算单 gross_revenue 按订单 SKU 的单件成交额解释；无订单 ID 的事前快照按 SKU 关联订单。", "采购、包装等结算单缺失的内部成本不计入公式通过判定。"]}}
