...
def request_from_models(product: Product, config: ProductPlatformConfig, activity: PromotionActivity) -> ProfitAnalysisRequest:
    ...
    return ProfitAnalysisRequest(
        product=ProductInput(name=product.name, sku=product.sku, purchase_cost=product.purchase_cost, packaging_cost=product.packaging_cost, weight_kg=product.weight_kg, volume_cm3=product.volume_cm3, currency=product.currency),
